...
For this project, you will be making a NEAR smart contract where you add words to a string, one at a time, to form a sentence. You are started with some boilerplate code and files, you should not need to change any of the boilerplate code.
...
You will need to have at least two different accounts (created when using `dev-deploy`) to fulfill the user stories. Rename one of the `neardev` accounts to use a `neardev-1` folder. The tests will use this to call your contract, you will likely want to use it as well. Once you have deployed and completely **finished** your contract, move that account info to a `neardev-final` folder.

**User Stories:**

1. Your smart contract should store a sentence in the form of a string.

1. Your contract should have an `init` method that accepts a `{ word }` parameter

1. Your contract should require initialization

1. Initializing your contract should concatenate the passed in `word` to the sentence

1. Your contract should have an `addWord` method that accepts a `{ word }` parameter

1. Calling the `addWord` method should add the `word` to the sentence with a space automatically added in front of the word
...
1. If your contract is "complete" it should return `The sentence is complete, more words cannot be added` when you call its `addWord` method

1. Your contract should have a `viewSentence` method that returns the sentence

1. You should keep track of all the accounts that added a word, and the word they added in the form of `{ id, word }`
...
The last test at the bottom deploys your `build/sentence-maker.wasm` contract file and runs the tests below, in the order they are listed, to test the methods on your contract. You cannot have a `neardev` folder when running the tests for this test to pass:
...
const learnDir = await __helpers.getDirectory('build-a-sentence-making-smart-contract');
...
const neardev1Dir = await __helpers.getDirectory('build-a-sentence-making-smart-contract/neardev-1');
...
const buildDir = await __helpers.getDirectory('build-a-sentence-making-smart-contract/build');
...
const contractId = await __helpers.getFile('build-a-sentence-making-smart-contract/neardev/dev-account');
const otherId = await __helpers.getFile('build-a-sentence-making-smart-contract/neardev-1/dev-account');
